    ...
    */
    ...
    public function viewApplication()
    {
        ...
        if(Auth::user())
        {
            ...
            $data['appliedJobs'] = [];
            $appliedJob = Auth::user()->appliedJob;
            foreach($appliedJob as $jobs)
            {
                $job = JobPost::find($jobs->job_id);
                if(Carbon::parse($job->end_date)>=Carbon::now())
                {
                    $data['appliedJobs'][] = $jobs;
                }
            }
    ...
    public function editApplication(Request $request, $jobId)
    {
        ...
        if(Auth::user())
        {
            ...
            if(!empty($appliedJob)){
                ...
                //Selected country and state if saved as a draft
                $selectedCountry = isset($appliedJob['country']) ? $appliedJob['country']['id'] : 0;
                $selectedState = isset($appliedJob['state']) ?$appliedJob['state']['id'] : 0;
                $selectedCity = isset($appliedJob['city']) ? $appliedJob['city']['id'] : 0;
            }else{
                //Selected country and state if going to aaply for first time
                $selectedCountry = $jobDetails['country_id'];
