...
class CompanyService
{
    ...
    */
    ...
    public function updateUserStatus($request)
    {
        ...
        if ($request['acc'] != 3) {
            ...
            User::where('id', $id)->update(['password' => null, 'status' => 0]);

            $candidateInfo =  User::where('id', $id)->delete();
        }
    ...
    */
    ...
    public function appliedCandidates($search = '')
    {
        ...
        $candidate = JobPost::where([['id', '=', $search['id']]])->with([
            'totalAppliedJob' => function ($query) use ($condition) {
