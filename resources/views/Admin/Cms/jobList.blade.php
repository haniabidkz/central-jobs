                            ...
                            <table class="table custom-table" id="candidateList">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$job['city']}}</td>
                                        <td>{{date('Y-m-d H:i a',strtotime($job['start_date']))}}</td>
                                        <td>{{date('Y-m-d H:i a',strtotime($job['end_date']))}}</td>
                                        <td>
                                        <?php if($job['status'] == 1){?> <button type="button" class="btn btn-success btn-sm">Active</button><?php }else{ ?> <button type="button" class="btn btn-danger btn-sm">Blocked</button><?php } ?>    
                                        </td>
                                        <td>
                                        <div class="dropdown">
                                            <button class="btn btn-sm btn-primary dropdown-toggle" type="button"  data-toggle="dropdown"  id="aaaaa">Actions <i class="fa fa-angle-double-down"></i></button>
                                                <ul class="dropdown-menu" id="bag" style="list-style-type: none;">
                                                    @if($job['status'] == 1)
                                                    <a class="dropdown-item status" href="javascript:void(0);"   data-id="{{$job['id']}}" data-val="{{$job['status']}}">Block</a>
                                                    @else
                                                    <a class="dropdown-item status" href="javascript:void(0);"  data-id="{{$job['id']}}" data-val="{{$job['status']}}">Unblock</a>    
                                                    @endif
